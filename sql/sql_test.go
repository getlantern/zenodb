...
`)
	...
	tableFields := core.Fields{knownField, oKnownField, xKnownField, lKnownField, pKnownField}
	...
	if assert.Len(t, fields, numFields) {
		...
		field = fields[idx]
		idx++
		expected = core.NewField("ptile3", PERCENTILE(DIV(myfield, CONST(10)), CONST(1), 0, 0, 1)).String()
		actual = field.String()
		assert.Equal(t, expected, actual)
