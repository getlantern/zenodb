package expr

// SUM creates an Expr that obtains its value by summing the given expressions
// or fields.
func SUM(expr interface{}) Expr {
	return &aggregate{"SUM", exprFor(expr), func(current float64, next float64) float64 {
		return current + next
	}}
}

// COUNT creates an Expr that counts the number of values.
func COUNT(expr interface{}) Expr {
	return &aggregate{"COUNT", exprFor(expr), func(current float64, next float64) float64 {
		return current + 1
	}}
}
