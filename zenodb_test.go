...
view_a:
  ...
  sql: >
	...
	for _, includeMemStore := range []bool{true, false} {
		...
		testHavingQuery(t, db, includeMemStore, epoch, resolution)
...
func testHavingQuery(t *testing.T, db *DB, includeMemStore bool, epoch time.Time, resolution time.Duration) {
	sqlString := `
SELECT i
FROM test_a
GROUP BY _
