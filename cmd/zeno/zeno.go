...
func main() {
	...
	flag.IntVar(&srv.ID, "id", 0, "unique identifier for a leader. if running in a cluster and omitting ID or specifying id = 0, you need to also specify the -allowzeroid flag")
	flag.BoolVar(&srv.AllowZeroID, "allowzeroid", false, "specify this flag to allow omitting the -id parameter or setting it to 0")
